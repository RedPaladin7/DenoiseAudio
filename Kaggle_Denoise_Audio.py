...
Conversion Date: 2025-10-06T13:03:53.798Z
"""
...
def spectral_convergence(true_audio, pred_audio):
    Y = tfs.stft(true_audio, WIN_LENGTH, HOP, N_FFT, window_fn=tf.signal.hann_window)
    P = tfs.stft(pred_audio, WIN_LENGTH, HOP, N_FFT, window_fn=tf.signal.hann_window)
    diff_mag = tf.abs(tf.abs(Y)-tf.abs(P))
    num = tf.sqrt(tf.reduce_sum(tf.square(diff_mag)))
    den = tf.sqrt(tf.reduce_sum(tf.square(tf.abs(Y)))) + eps()
    return num / den
...
def si_sdr(true_audio, pred_audio):
    x = true_audio # Clean (True)
    s = pred_audio # Enhanced (Predicted)
    
    # Center signals
    x_zm = x - tf.reduce_mean(x, axis=-1, keepdims=True)
    s_zm = s - tf.reduce_mean(x, axis=-1, keepdims=True) 

    # Calculate projection of s_zm onto x_zm (signal component)
    # The scalar alpha is calculated first, ensuring keepdims=True for broadcasting.
    ...
    # Error component (noise + distortion)
    e = s_zm - proj
    
    # Calculate power (L2 norm squared)
    signal_power = tf.reduce_sum(proj**2, axis=-1)
    error_power = tf.reduce_sum(e**2, axis=-1)
    
    # Ensure numerical stability for the log argument
    ratio = (signal_power + eps()) / (error_power + eps())
    
    # Calculate SI-SDR in dB: 10 * log10(ratio)
    si_sdr_val = 10.0 * tf.math.log(ratio) / tf.math.log(10.0)
    
    return si_sdr_val

def si_sdr_loss(true_audio, pred_audio):
    # Loss is the negative mean SI-SDR
    return -tf.reduce_mean(si_sdr(true_audio, pred_audio))
...
def total_loss(true_audio, pred_audio):
    return si_sdr_loss(true_audio, pred_audio)
...
class EMA:
    def __init__(self, model, decay=EMA_DECAY):
        ...
        self.shadow = [tf.Variable(tf.identity(w)) for w in model.weights]
    ...
    def apply_to(self, model):
        self.backup = [tf.Variable(tf.identity(w)) for w in model.weights] # temporary backup
...
for epoch in range(1, EPOCHS+1):
    ...
    for step, (noisy, clean) in enumerate(train_ds.take(STEPS_PER_EPOCH), start=1):
        ...
        global_step += 1
        step_tensor = tf.constant(global_step, dtype=tf.int64)
        current_lr = lr_schedule(step_tensor).numpy()
        if step % 100 == 0:
