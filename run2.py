...
# -*- coding: utf-8 -*-
"""
Simplified Denoising U-Net and ResNet Comparison (Corrected)
"""
...
# **FIX:** Define spectrogram dimensions as constants
...
def read_wav_mono(path, target_sr=SR):
    ...
    if y.ndim == 2:
        y = y.mean(axis=1)
    # The original tf.audio.resample was removed for simplicity in this notebook context
    # If resampling is strictly needed, it should be done here using standard tools.
...
def create_model_output_layers(mask_output, mag_input, phase_input, name):
    ...
    """
    # **FIXED:** Use pre-calculated constants
    ...
    # Output shape must be correctly set
...
def resnet_block(x, filters, kernel_size=(3, 3), name='res_block'):
    ...
    # 3. Projection shortcut (1x1 Conv) if input/output dimensions differ
...
def build_resnet():
    ...
    # Final output uses the shared logic
...
def build_unet():
    ...
    # Cropping to match d2 (Important for U-Net connections)
...
# ## Optimizer (FIXED: Separate optimizers for separate models)
...
# FIXED: Separate optimizers for each model to avoid Keras Autograph error.
...
print(f"Training two models (UNet and ResNet) for comparison.")

# Initial print for GPU status (kept from original code)
...
for epoch in range(1, EPOCHS+1):
    ...
    for step, (noisy, clean) in enumerate(train_ds.take(STEPS_PER_EPOCH), start=1):
        # Training step for both models simultaneously
...
# Choose the better model for visualization based on this sample's SI-SDR
...
# Determine the overall best model based on final validation SI-SDR
...
best_ema.apply_to(best_model) # Apply best weights before export
...
# Note: This will only succeed if the model was trained successfully.
try:
...
best_ema.restore(best_model) # Restore to original state after export

print("\nReady! Use: denoise_file('/kaggle/input/your.wav', out_path='/kaggle/working/clean.wav', show=True)")

# Placeholder for denoise_file (needs to be redefined to use the final best model)
